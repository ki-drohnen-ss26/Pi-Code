...
def make_config(argv: list) -> Config:
    ...
    * Simulation accidentally running REAL values fails **immediately and harmlessly**:
    ...
    # On a no-baro flow-only build the EKF never reports a position estimate on the
    # ground, so a ground start would deadlock in wait_ready_to_arm - the pilot flies the
    # first metre, the companion takes over in the air (and its rangefinder/EKF cross-gate
    # refuses a diverged filter). Force it for the REAL profiles only: SITL keeps a
    # settling EKF that comes up on the floor, so simulation must still be able to arm
    # itself and rehearse the full arm-and-takeoff path.
    if not ("--sim" in argv or "--sitl" in argv):
...
def log_profile(config: Config) -> None:
    ...
    """
    # Variant banner FIRST, so every log names the firmware this code targets. On
    # 4.8.0-dev the parameter NAMES differ (RTL_ALT_M, RNGFND1_MIN/MAX, MAV_GCS_SYSID),
    # and a wrong name is silently ignored by the FC - so which variant produced a log
    # is not a detail, it is the difference between the params taking effect or not. And
    # this is the NO-BARO build: the rangefinder is the ONLY height source, which is why
    # the strict altitude-drift protocol in README_VARIANT.md is not optional here.
    log.warning("[PROFILE] VARIANT fw480dev-nobaro: ArduCopter 4.8.0-dev, NO-BARO build -")
    log.warning("[PROFILE] rangefinder is the only height source, strict protocol applies "
                "(README_VARIANT.md)")
